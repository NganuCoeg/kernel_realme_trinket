...
export KBUILD_BUILD_USER=renn # Change with your own name or else.
export KBUILD_BUILD_HOST=hptolol # Change with your own hostname.
...
function compile() {
  ...
  make -j$(nproc) ARCH=arm64 O=out \
   ...
   if ! [ -a "$IMAGE" ]; then
    ...
    git clone --depth=1 https://github.com/NganuCoeg/AnyKernel3 AnyKernel
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Compile took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Realme Trinket Series (RMX1911/RMX1925/RMX1927/RMX2030)</b> | <b>$(${CLANG_ROOTDIR}/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')</b>"
